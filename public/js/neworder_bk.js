	...
	jQuery.fn.onChangeMyCustomSelect = function(obj, parent_name, selected_val, order_type){
		...
		$("div[parent='"+parent_name+"'][parent_value='depend'][order_type='"+order_type+"']").each(function(){
			...
			else
			{
				...
				if(ord_type=='Web' && f_name=='Type')
				{


					//Rich Media Delivery Restriction
					...
					//{
						...
						select_obj.find('option').each(function(){
							...
							{						
								if($(this).val() != 'Other' && $(this).val() != '2nd Day' && $(this).val() != 'Overnight' && $(this).val() != 'Overnight10'){
					...
					//}
					//Rich Media Delivery Restriction Ends
	...
	jQuery.fn.cleanOrderFields = function(){		
		...
		$("#4").autocomplete({
			...
			open: function() {
				...
				$('.ui-menu').width(255);
				$('.ui-menu').css({'left':'12px','top':'33px','position':'absolute','z-index':'10'});
		...
		$("[name='Size Change Only']").each(function(){
			...
			$(this).unbind().change(function(){			
				...
				ord_type = $(this).parent().parent().attr('order_type');
				...
				if($(this).val()=='Yes')
				{
					...
					if(selected_versions>existing_versions)
					{
						...
						{
							div_obj1 = $("<div parent='Size Change Only'  parent_value='Yes' order_type='"+ord_type+"' style='width:900px;clear:both;' ver='"+(mul+1)+"'><div class='m-block t-right'><h2>VERSION "+(mul+1)+"</h2></div></div>");
		...
		$("#formSubmit").click(function(event){
			...
			$("#order_info_form").find("input:checked[name='Order Type']").each(function(){				
				...
				if(parseInt(additional_version)>0 && size_change_only=='Yes')
				{					
		...
		$("#formSave").click(function(event){
			...
			$("#order_info_form").find("input:checked[name='Order Type']").each(function(){				
				...
				if(parseInt(additional_version)>1 && size_change_only=='Yes')
				{					
					form_multi_spec_details = $("#ord_form_multiver_"+$(this).val()).serialize();
