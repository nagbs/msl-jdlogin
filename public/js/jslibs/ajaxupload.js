...
(function($)
{
    ...
    {    	
    	...
    	*/
		...
		params.push('ax-temp-path=' + settings.tmp_path);
    ...
    {    	
	    ...
	    name = removeSpecialCharFromFileName(name) ;
    ...
    /*
     * Remove Special Char From FileName
     * Ajai
     * 
     */
    function removeSpecialCharFromFileName (fileName ){
    	
    	var tmpFileNameArray = fileName.split(".");
    	var fileExt = fileName.split('.').reverse()[0];
    	
    	if (tmpFileNameArray.length > 2) {
    		FileNameWithoutExt = tmpFileNameArray[0] + '_' + tmpFileNameArray[1] ;
    	} else {
    		FileNameWithoutExt = tmpFileNameArray[0];
    	}
    	
    	//string = string.replace(/[&\/\\#,+()$~%.'":*?<>{}]/g,'_');
    	FileNameWithoutExt = FileNameWithoutExt.replace(/ +?/g, '_');
    ...
    {
    	//var fn     = fileobj.name;
    	// To Remove Speial charecter from FileName
    	var fileName = fileobj.name;
    	var fn		= removeSpecialCharFromFileName(fileName) ;
		
    	var size	= sizeFormat(settings.showSize, fileobj.size);
	//Added By Ajai (condition for small size file in bytes which one was showing 0 mb )
	if (size == '0 mb'){
    		size	= sizeFormat('bytes', fileobj.size);
    	} else {
	...
	//-----
    	...
    	if(settings.showOrderType=='true')
    	{
    	...
    	{
    		var orderTypeCont = $('<div style="display:none"/>');
	    	
	    	orderTypeFields = "";   	
	    			    		
    		orderTypeFields = orderTypeFields + '<input type="hidden" name="order_components['+fn+'][]" />';   		
	    ...
	    {
	    	var nameC	= $('<div class="ax-file-name single-column attache-txt-field">'+fn+'</div>').appendTo(li);
	    }
	    ...
	    {
	    	var instr	= $('<div class="instruction"><input type="text" name="fileinstr['+fn+']" class="o-input-m" style="margin-top:2px;" placeholder="instructions"></div>').appendTo(li);
	    }
	    ...
	    var pNum	= $('<div class="ax-progress-info">0%</div>').appendTo(pBar);
    ...
    function ajaxList(list, obj, settings)
    {
    	...
    	obj.end = function(name, size, status, info)
	    {
	    	...
	    	this.tools.pNum.html('File Attached');
	    	this.tools.pBar.parent().css({'box-shadow':'none','border':'none','background':'none'});
	    	this.tools.pBar.css({'background':'none'});
		...
		fileUploadCount--;
	    	...
	    	if((fileUploadCount==0 && jobSubmit==1) || $('#file_upload_completion').val()=='1')
	    		{
	...
	var methods =
	{
		init : function(options)
		{
    	    ...
    	    {	
				...
				var fieldSet  	= $('<fieldset />').append('<legend class="ax-legend"><h2>ORDER COMPONENTS</h2></legend>').appendTo($this);
			    ...
			    */
				...
				var dragdrop = $('<div class="" style="padding-top:2px;" id="dropArea"><!--<img src="/images/dragdrop.png" style="vertical-align:middle;" >&nbsp; --><b>Drag & Drop Files</b></div><br/>').appendTo(fieldSet);
