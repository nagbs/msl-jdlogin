...
{	
	...
	if(fileUploadCount==0 && formSave==1)
    {
		formSave = 0;
...
{	
	function fileTemplate(list, fileobj, settings)
	{
    	...
    	else if(settings.page=='ReviseOrder')
    	{
	    	
	    	$("#ReviseForm").find("[name='Revision_Order_Type']").each(function(){
	    			//alert($(this).val());
	    			selected_order_types[otypecnt++] = $(this).val();
	    	});
    	}
    ...
    function dropList(list, obj, settings)
    {
		var FILES	= settings.FILES;
    	var tools	= fileTemplate(list, obj, settings);
    	...
    	var progress = tools.pBar;
    	
    	var pbar = $('<div id="progress_container" style="float:left"><div id="progress_bar" style="float:left;"></div></div><div id="progress_completed" style="padding-top:5px;float:left;vertical-align:middle;position:absolute;" class="grey12"></div>');
    	
    	progress.append(pbar);
    	
    	pbar.fadeIn(100);
    	var pgr = pbar.find('#progress_bar');
    	var pgrd = pbar.find('#progress_completed');
		...
		xhr.onreadystatechange = function(e) {
			if (xhr.readyState == 4) {
				...
				progress.parent().parent().append(remove);
		...
		// start upload
		xhr.open("POST", "/upload_dragdrop.php", true);
		xhr.setRequestHeader("X_FILENAME", file.name);
		xhr.setRequestHeader("X_TMPPATH", tmp_path);
		xhr.send(file);
	...
	$.fn.myFileUpload = function(options)
	{
		...
		return this.each(function(){			
			...
			var drop_area = $('<div id="dropArea" style="height:80px;">Drag & Drop Files here</div>').appendTo($(this));
			...
			{
				drop_area.css({width:settings.width});
			}
