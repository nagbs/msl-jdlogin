...
function createOrderListHTMLData(data, clearbody) {	
	...
	{	
		$.each(data,function(jobNo,jobDetails)
		{
			$.each(jobDetails,function(fieldKey, fieldValue)
			{
				...
				if(fieldKey!='Bkgnd_color' && fieldKey!='JobNo' && fieldKey!='Totalrows' && fieldKey!='Action_items' && fieldKey!='Deliverypritype' && fieldKey!='Revisioncount' && fieldKey!='Deliverytype'  && fieldKey!='Creationsource')
				{
					headTitle = $('<div />');
					if(fieldKey=='Job_no')
					{
	...
	if(def===undefined)
	{
		...
		reloadDiv.attr('class','actions-col');
		reloadDiv.attr('style','padding-top:0px;margin-top:-17px;margin-right:-5px;cursor:pointer;');
		reloadDiv.html('<span id="JobCount">Showing <span id="displaying"></span> of <span id="total">'+TotalJobs+'</span></span><img src="/images/reload.png" >');	
	...
	$.each(data,function(jobNo,jobDetails)
	{
		...
		if(jobDetails.Creationsource=='1')
		{
			statusDiv.attr('class','status-col ord-int');
		}
		else if(jobDetails.Creationsource=='2')
		{
			statusDiv.attr('class','status-col ord-mail');
		}
		else
		{
			//statusDiv.attr('class','status-col ord-system');
		...
		$.each(jobDetails,function(fieldKey, fieldValue)
		{
			...
			if(fieldKey!='Bkgnd_color' && fieldKey!='Status' && fieldKey!='JobNo' && fieldKey!='Totalrows' && fieldKey!='Action_items' && fieldKey!='Deliverypritype' && fieldKey!='Revisioncount'  && fieldKey!='Deliverytype'  && fieldKey!='Creationsource')
			{
				if (fieldKey == 'Submit_by') {
