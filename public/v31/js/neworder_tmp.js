...
//		alert(site);	 
		...
		if(obj.attr('name')=="Build Platform")
		{
			...
			//$('div[title="Click Tag"][order_type="Web"]').hide();
			$('[name="Click Tag"]').parent().parent().parent().parent().hide();
                        $('[name="Click Tag"]').parent().parent().parent().parent().prev().find('label').html('');

			$('<span id="filesmsg" style="position:absolute;margin-top:3px;padding-top:3px;font-weight:bold">* Add 3 images including customer logo</span>').appendTo($('.ax-button').parent());
	...
	jQuery.fn.cleanOrderFields = function(){		
		...
		$("[name='Size Change Only']").each(function(){
			...
			$(this).unbind().change(function(){			
				...
				parent_1 = $(this).parent().parent().parent().parent().attr('parent');				
				
				if($(this).val()!='Different Creative')
				{	
					selected_versions = $("div[order_type='"+ord_type+"'] [name='"+parent_1+"']").val();
