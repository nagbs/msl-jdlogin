$(document).ready(function(){
	...
	$("#26").change(function(){
		...
		var widthsize, depthsize, total_size1, topbleed, rightbleed;
		...
		if($.isNumeric(topbleed))
		{
			total_size1 = parseFloat(depthsize) + (parseFloat(topbleed)*2);
		}

		if($.isNumeric(rightbleed))
		{
			total_size = parseFloat(widthsize) + (parseFloat(rightbleed)*2);
		}


		if(!($.isNumeric(topbleed)) && !($.isNumeric(rightbleed)))
		{
			total_size = page_size;
		}
		else if($.isNumeric(topbleed) && !($.isNumeric(rightbleed)))
		{
			total_size = parseFloat(widthsize)+'X'+total_size1;
		}
		else if(!($.isNumeric(topbleed)) && $.isNumeric(rightbleed))
		{
			total_size = total_size+'X'+parseFloat(depthsize);
		}
		else
		{
	...
	$("#27").change(function(){
		var widthsize, depthsize, total_size, total_size1, topbleed, rightbleed;
		var page_size = '', total_size='';
	...
	$("#185").change(function(){
		var pagesize, rightbleed, topbleed, widthsize, depthsize, total_size, total_size1;
		...
		if($.isNumeric(widthsize) && $.isNumeric(rightbleed))
		{
			total_size = parseFloat(widthsize) + (parseFloat(rightbleed)*2);
		}
		else if($.isNumeric(widthsize) && !($.isNumeric(rightbleed)))
		{
			total_size = parseFloat(widthsize);
		}
		else if(!($.isNumeric(widthsize)) && $.isNumeric(rightbleed))
		{
			total_size = parseFloat(rightbleed)*2;
		}

		if($.isNumeric(depthsize) && $.isNumeric(topbleed))
		{
			total_size1 = parseFloat(depthsize) + (parseFloat(topbleed)*2);
		}
		else if($.isNumeric(depthsize) && !($.isNumeric(topbleed)))
		{
			total_size1 = parseFloat(depthsize);
		}
		else if(!($.isNumeric(depthsize)) && $.isNumeric(topbleed))
		{
			total_size1 = parseFloat(topbleed)*2;
		}
	...
	$('#Bleed').change(function(){
		if($('#Bleed :selected').text()=='No')
		{
			$('#184').val('');
			$('#185').val('');
			$('#26').trigger("change");
	...
	if($('#Bleed :selected').text()=='No')
	{
		$('#Bleed').trigger("change");
	}
	else if($('#Bleed :selected').text()=='Yes')
	{
		$('#26').trigger("change");
	}
});
