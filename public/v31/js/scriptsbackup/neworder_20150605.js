...
//		alert(site);	 
		...
		if(obj.attr('name')=="Build Platform")
		{
			...
			if(n>=5 && n<=10)
			{
				...
				$('[title="Delivery Date"] .custRef').html('3 PM').removeClass('select_grey_out');
			...
			else
			{
				...
				$('[title="Delivery Date"] .custRef').html('Overnight').removeClass('select_grey_out');
				$('#14').trigger('change');
			}
		    }
    		    else
		...
		$("div[parent='"+parent_name+"'][parent_value='depend'][order_type='"+order_type+"']").each(function(){
			...
			if($('[name="Build Platform"]').val()=='2adpro Adbuilder' && $(this).attr('title')=='Type')
        		{
		                return;
        		}	
...
if(site=='Media Services(Auckland)' && ord_type=='Web' && f_name=='Type')
{
	...
	if($('[name="Build Platform"]').val()=='2adpro Adbuilder')
	{
		$('div[title="Type"][order_type="Web"]').hide();
	}
