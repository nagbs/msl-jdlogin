...
{	
	...
	if(fileUploadCount==0 && magazineSubmit==1)
    {
        $('#magazineformSubmit').trigger('click');
    }
	
	if(fileUploadCount==0 && magazineSubOrderSubmit==1)
    {
        $('#subpagesformSubmit').trigger('click');
    }
	
	if(fileUploadCount==0 && singleSubOrderSubmit==1)
    {
        $('.subpage_submit').trigger('click');
    }
}
...
{	
	function fileTemplate(list, fileobj, settings)
	{
    	...
    	else if(settings.page=='MagazineSubPages')
    	{	
    		$("#magazine_sub_pages_main_form").find("[name='Order Type']").each(function(){
    			//alert($(this).val());
    			selected_order_types[otypecnt++] = $(this).val();
    		})
    		;
    		formidentifier = "MagazineSubPages";
    	}
    	else
    	{
    	...
    	{    	
    		...
    		{
    			if(formidentifier=='MagazineSubPages')
        		{
				orderTypeFields = orderTypeFields + '<li><input type="checkbox" style="display:none;" checked="checked" value="'+selected_order_types[oi]+'" name="'+settings.formid+'['+fn+'][]"  id="'+selected_order_types[oi]+'" /></li>';

        			/*orderTypeFields = orderTypeFields + '<li><input type="checkbox" style="display:none;" checked="checked" value="'+selected_order_types[oi]+'" name="'+settings.formid+'['+fn+'][]"  id="'+selected_order_types[oi]+'" /> <a href="#" id="o-'+selected_order_types[oi]+'" rel="order_type" class="o-'+selected_order_types[oi]+'-active" onclick="" >'+selected_order_types[oi]+'</a></li>';*/
        		}
    			else
    			{
