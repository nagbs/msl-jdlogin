...
var isCampaignOrderList = 0;
var isCampaignOrderListPrevVal = 0;
...
//$('#'+id+' .contentarea').html(textStatus);
			...
			$('#emailProof_'+jobID).click(function(){	
				...
				//getting version
				$('#emailProofVersionDialog').find('#jobno').html(jobID);
				//--
				$.ajax({
					url: '/orders/getversiondetail',
					type: 'POST',
					data: "form_action=getversioninfo&order_number="+jobID,
					dataType: 'json',
					beforeSend: function() {
						//
					},
					success: function(jsondata) {
							...
							$('.webverchk').html('');
							$('.mobileverchk').html('');
							$('#actionMenuEmailproofversion #printver').hide();
							$('#actionMenuEmailproofversion #webver').hide();
							$('#actionMenuEmailproofversion #mobilever').hide();

							$.each(jsondata.version_detail,function(key,val) {
								$chk_box_str = '';
								$vers_det = val.split(',');
								readonly = '';
								if($vers_det.length=='1')
								{
								...
								if(key=='1')
								{
									for(i=0; i<$vers_det.length; i++)
									{
										
										$chk_box_str += "<input type='checkbox' name='printselver[]' class='printselver' value="+$vers_det[i]+" checked"+readonly+">"+$vers_det[i];
									}
									
									$('.printverchk').html($chk_box_str);
									$('#actionMenuEmailproofversion #printver').show();
								}
								else if(key=='2')
								{
									for(i=0; i<$vers_det.length; i++)
									{
										$chk_box_str += "<input type='checkbox' name='webselver[]' class='webselver' value="+$vers_det[i]+" checked='checked'"+readonly+">"+$vers_det[i];
									}
									
									$('.webverchk').html($chk_box_str);
									$('#actionMenuEmailproofversion #webver').show();
								}
								else if(key=='4')
								{
									for(i=0; i<$vers_det.length; i++)
									{
										$chk_box_str += "<input type='checkbox' name='mobileselver[]' class='mobileselver' value="+$vers_det[i]+" checked='checked'"+readonly+">"+$vers_det[i];
									}
									
									$('.mobileverchk').html($chk_box_str);
									$('#actionMenuEmailproofversion #mobilever').show();
								}
							});
							
							geteproofversion(jobID);
							//getting version
					},
					error: function(errorThrown) {
						//$('#'+id+' .contentarea').html(textStatus);
	...
	$.each(clickIDArrays, function(){
		$("#"+this).unbind().click(function(){
			...
			if(jobIDArrays[0] == 'subactionMenu')
			{
				...
				//$("#"+popupID+" ul").append('<li><a href="javascript:void(0);" class="addmorepage" id="addMorePage_'+jobID+'" ><img src="/images/change.png" />Add More Pages</a></li>');
			}
			...
			$('#addMorePage_'+jobID).click(function(){
				$('#addMorePageDialog').find('#jobno').html(jobID);
				$('#requester').val(submittedBy);
				addMoreMagazinePages(jobID);
			});
 ...
 * @return null.
*/
function showPopupForCampaignJobs(clickID , popupID)
{
	var clickIDArrays = clickID.split(",")
	var status;
	var relJobDetailsWithSeparator;
	...
	$.each(clickIDArrays, function(){
		$("#"+this).unbind().click(function(){
			...
			if(jobIDArrays[0] == 'subactionMenu')
			{
				...
				//$("#"+popupID+" ul").append('<li><a href="javascript:void(0);" class="addmorepage" id="addMorePage_'+jobID+'" ><img src="/images/change.png" />Add More Orders</a></li>');
			}
			...
			$('#addMoreOrder_'+jobID).click(function(){
				$('#addMorePageDialog').find('#jobno').html(jobID);
				$('#requester').val(submittedBy);
				addMoreCampaignOrder(jobID, campaignId);
			});
			
			$('#changeJob_'+jobID).click(function(){
				checkSessionActive();
				//$.post('/campaign/createcampaign',{"jobno":jobID},function(data){
				$.post('/orders/create',{"jobno":jobID},function(data){	
					$('.content-main-left').css({"width" : '900px'});
					$('#dashboard').remove();
					$('#orderlist').remove();
...
function createMenuDivHtml(status,jobID)
{
	...
	var addMorePagesLI = '<li><a href="#" class="addmorepage" id="addMorePage_'+jobID+'" ><img src="/images/change.png" />Add More Pages</a></li>';
	var addMoreOrdersLI = '<li><a href="#" class="addmoreorder" id="addMoreOrder_'+jobID+'" ><img src="/images/change.png" />Add More Orders</a></li>';
	/*
    ...
    for(var i=0;i<actionItemsArray.length;i++)
    {
    	...
    	if(actionItemsArray[i]=='ADD_MORE_PAGES')
        {
                createMenuDivHtml += addMorePagesLI;
        }
		if(actionItemsArray[i]=='ADD_MORE_ORDERS')
	    {
	            createMenuDivHtml += addMoreOrdersLI;
	    }
...
function geteproofversion(jobno)
{
	//New Popup Box
	$("#msgbox_text").html('');	
	$("#msgbox_text").html($( "#dialog-emailproofversion" ).html());
	$("#msgbox").css('display','table');
	$("#msgbox").fadeIn('fast',function(){
		$("#msgbox").css('display','table');
	});
	...
	var $print_selected_version = [];
	var $web_selected_version = [];
	var $mobile_selected_version = [];
	
	$('#msgbox_text').find('#actionMenuEmailproofversion #submit_form').click(function(e){
		//e.preventDefault();
		
		$('#msgbox_text').find('#actionMenuEmailproofversion').find('input.printselver:checked').each(function() 
		{
			$print_selected_version.push($(this).val());
		});
		
		$('#msgbox_text').find('#actionMenuEmailproofversion').find('input.webselver:checked').each(function() 
		{
			$web_selected_version.push($(this).val());
		});
		
		$('#msgbox_text').find('#actionMenuEmailproofversion').find('input.mobileselver:checked').each(function() 
		{
			$mobile_selected_version.push($(this).val());
		});
		
		$("#msgbox_text").find("#actionMenuEmailproofversion").submit();
	});	
	
	$("#msgbox_text").find("#actionMenuEmailproofversion").validate({
		submitHandler: function(form) {
			checkSessionActive();
			...
			if($('#msgbox_text').find('#changeTo').val() != 'Select Owner') {
				...
				$.ajax({
					...
					type: 'POST',
					async: false,
					data: "form_action=eproof&jobno="+jobno+"&printselectedversion="+$print_selected_version+"&webselectedversion="+$web_selected_version+"&mobileselectedversion="+$mobile_selected_version,
					dataType: 'json',
					beforeSend: function() {
...
function reOrderingPages(jobno)
{
	...
	$('#'+parent_job_div_id).find('.content').children('.content-left-box-inner').each(function(){
		...
		$("#sortable ul").append('<li><div class="pageNo">'+page_no+'</div><div class="transactionNo">'+track_no+'</div><div class="jobNo">'+job_no+'</div><div class="newPageNo">'+page_no+'</div><input class="newtrackpageno" type="hidden" value="'+page_no+'" name="'+track_no+'"></li>');
	});
...
function addMoreMagazinePages(jobno)
{
	var flag = false ;
	
	$("#msgbox_text").html('');	
	$("#msgbox_text").html($( "#dialog-addmorepagejob" ).html());
	$("#msgbox").css('display','table');
	$("#msgbox").fadeIn('fast',function(){
		$("#msgbox").css('display','table');
	});

	$('#msgbox_text').find('#addmorepage_form').click(function(e){
		e.preventDefault();
		$("#msgbox").fadeOut('fast');
	});
	
	$('#msgbox_text').find('#submit_addpage_form').click(function(e){
		e.preventDefault();
		$("#msgbox_text").find("#actionMenuAddmorepage").submit();
	});
	

	$("#msgbox_text").find("#actionMenuAddmorepage").validate({
		submitHandler: function(form) {
			checkSessionActive();

			$.post('/magazines/addmorepage', {
				jobno: jobno,
				no_of_pages: $("#msgbox_text").find('#no_of_pages').val(),                 
				form_action: 'add_more_page'
			}, function(d){
				//console.log(d);
				$("#msgbox").fadeOut('fast');
				loadOrderList();
...
function addMoreCampaignOrder(jobno, campaign_id)
{
	var flag = false ;
	
	checkSessionActive();

	$.post('/campaigns/createcampaign', { jobno: jobno, campaign_id : campaign_id, camapign_add_more_order: '1' }, function(data){
		$('.container').hide('slow');
	...
	*/
	...
	$('#msgbox_text').find("#actionMenuEmailproof").validate({
		submitHandler: function(form) {			
			...
			var Multiple_email = new Array;
	    		Multiple_email.push($("#msgbox_text").find('#To').val());
	    		 $("#msgbox_text").find('#mulTo').each(function(){
	    			 	var mul_email = $(this).val();
	    			 	if(mul_email != '')
	    			 	{
	    			 		Multiple_email.push(mul_email);
	    			 	}
	    			});
	    		 
	    		var email_id = Multiple_email.join(';');
	    		mailto_link='mailto:'+email_id+'?subject='+rawurlencode($("#msgbox_text").find('#Subject').val())+'&body='+rawurlencode($("#msgbox_text").find('#Message').val())+'&signature=';
...
function revise()
{
		...
		else if(isCampaignOrderList == '1')
		{
			$('.hdng .activeJobTab.magazinePage a[rel="activeCampaign"]').show();
		}
		else
		{
 ...
 * @return null.
*/
function messageJob(jobID, reloadJobList)
{
	if(reloadJobList==1)
	{
		loadOrderList();
...
/**
 * Function (suborderJob) is used to show suborder section.
 * 
 * @return null.
 */
function suborderJob(jobID)
{
	jobno = jobID;
	$('#activelist').val('ver');
	$("#isloading").val("true");
...
function getActiveTabs() {
	$.post('/view/getactivetabs', function(data) {
		activeTabs = data;
	...
	if($('[name="activeJobsTab"]').val()==2)
    {
		isMagazineOrderList = '1';
		isCampaignOrderList = '0';
    }
	else if($('[name="activeJobsTab"]').val()==3)
    {
        isCampaignOrderList = '1';
...
function loadOrderList() {
	...
	{
		$.post('/view/campaignlist', function(data,textStatus, jqXHR) {		
			createCampaignListHTMLData(data, '');
		});
	}
	else
	{
...
function statusStyleClass(statusData) {
	...
	} else if(statusData == 'Editorial Approved'){
                statusDivclass = 'status color9' ;
        } else if(statusData == 'Editorial Proof'){
	    statusDivclass = 'status color7' ;
	} else if(statusData == 'On Hold'){
                statusDivclass = 'status color10' ;
        }
...
function createOrderListHTMLData(data, clearbody) {	
	...
	if(isCampaignOrderListPrevVal == 1)
	{*/
		$("#activelist").remove();
	//}
	...
	if(def===undefined)
	{
		...
		if(activeTabs == '4')
		{
			headingDiv.html("<h4 class='activeJobTab magazinePage'> <a class='sel' rel='activeAds' href='javascript:void(0);' id='activeAds'>Active Ads</a> <a rel='activeCampaign' href='javascript:void(0);' class='' id='activeCampaign'>Active Campaign</a> <a rel='activeMagazine' href='javascript:void(0);' class='' id='activeMagazine'>Active Magazine</a> </h4> <div class='rt'><div id='jobs_reload'><span id='JobCount'>Showing <span id='displaying'></span> of <span id='total'></span></span></div></div>");
		}
		else if(activeTabs == '3')
		{
			headingDiv.html("<h4 class='activeJobTab magazinePage'> <a class='sel' rel='activeAds' href='javascript:void(0);' id='activeAds'>Active Ads</a> <a rel='activeCampaign' href='javascript:void(0);' class='' id='activeCampaign'>Active Campaign</a> </h4> <div class='rt'><div id='jobs_reload'><span id='JobCount'>Showing <span id='displaying'></span> of <span id='total'></span></span></div></div>");
		}
		else if(activeTabs == '2')
		{
			headingDiv.html("<h4 class='activeJobTab magazinePage'> <a class='sel' rel='activeAds' href='javascript:void(0);' id='activeAds'>Active Ads</a> <a rel='activeMagazine' href='javascript:void(0);' class='' id='activeMagazine'>Active Magazine</a> </h4> <div class='rt'><div id='jobs_reload'><span id='JobCount'>Showing <span id='displaying'></span> of <span id='total'></span></span></div></div>");
		}
		else
		{
			headingDiv.html("<h4 class='activeJobTab magazinePage'> <a class='sel' rel='activeAds' href='javascript:void(0);' id='activeAds'>Active Ads</a> </h4> <div class='rt'><div id='jobs_reload'><span id='JobCount'>Showing <span id='displaying'></span> of <span id='total'></span></span></div></div>");
		}
	...
	{	
		$.each(data,function(jobNo,jobDetails)
		{
			$.each(jobDetails,function(fieldKey, fieldValue)
			{
				...
				if(fieldKey!='Bkgnd_color' && fieldKey!='JobNo' && fieldKey!='Totalrows' && fieldKey!='Action_items' && fieldKey!='Deliverypritype' && fieldKey!='Revisioncount' && fieldKey!='Deliverytype' && fieldKey!='Siteordercustomstatus')
				{
					headTitle = $('<div />');
					if(fieldKey=='Job_no')
					{
	...
	$.each(data,function(jobNo,jobDetails)
	{
		...
		$.each(jobDetails,function(fieldKey, fieldValue)
		{
			...
			if(fieldKey!='Bkgnd_color' && fieldKey!='Status' && fieldKey!='JobNo' && fieldKey!='Totalrows' && fieldKey!='Action_items' && fieldKey!='Deliverypritype' && fieldKey!='Revisioncount'  && fieldKey!='Deliverytype' && fieldKey!='Suborderpageno' && fieldKey!='Siteordercustomstatus')
			{
				if (fieldKey == 'Submit_by') {
	...
	$('#activeMagazine').unbind().click(function(){
		//isMagazineOrderList = 1;
		//isMagazineOrderListPrevVal = 0;
		$('[name="activeJobsTab"]').val(2);
		$('.inp-text1').trigger('click');
		//loadOrderList();
	});
	
	$('#activeCampaign').unbind().click(function(){
		//isMagazineOrderList = 1;
		//isMagazineOrderListPrevVal = 0;
		$('[name="activeJobsTab"]').val(3);
		$('.inp-text1').trigger('click');
		//loadOrderList();
	});
...
function loadMoreOrderListHTMLData(data, clearbody) {
	...
	$.each(data,function(jobNo,jobDetails)
	{
		...
		$.each(jobDetails,function(fieldKey, fieldValue)
		{							
			if(fieldKey!='Bkgnd_color' && fieldKey!='Status' && fieldKey!='JobNo' && fieldKey!='Totalrows' && fieldKey!='Action_items' && fieldKey!='Deliverypritype' && fieldKey!='Revisioncount'  && fieldKey!='Deliverytype' && fieldKey!='Siteordercustomstatus')
			{
				if (fieldKey == 'Submit_by') {
					jobSubmittedBy = fieldValue;
				}
...
function createMagazineListHTMLData(data, clearbody) 
{
	...
	if(def===undefined)
	{
		...
		{
			headingDiv.html("<h4 class='activeJobTab magazinePage'> <a class='' rel='activeAds' href='javascript:void(0);' id='activeAds'>Active Ads</a> </h4> <div class='rt'><div id='jobs_reload'><span id='JobCount'>Showing <span id='displaying'></span> of <span id='total'></span></span></div></div>");
		}
	...
	{	
		$.each(data,function(jobNo,jobDetails)
		{
			$.each(jobDetails,function(fieldKey, fieldValue)
			{
				...
				if(fieldKey!='Bkgnd_color' && fieldKey!='JobNo' && fieldKey!='Totalrows' && fieldKey!='Action_items' && fieldKey!='Deliverypritype' && fieldKey!='Revisioncount' && fieldKey!='Deliverytype' && fieldKey!='Suborderpageno' && fieldKey!='Siteordercustomstatus')
				{
					headTitle = $('<div />');
					if(fieldKey=='Job_no')
					{
	...
	$.each(data,function(jobNo,jobDetails)
	{
		...
		{	
			jobDetails.Action_items = 'ADD_MORE_PAGES,'+jobDetails.Action_items;
		}
		var actionMenuDiv = $('<div/>');
		actionMenuDiv.attr('class','actions-col');
		actionMenuDivHtml = '<div class="open" title="Open Sub Order">Open</div>';
		...
		$(this).find('.open').click(function(){
			...
			$.post('/view/magazinelist', {'parent_jobno':parentjobno}, function(data,textStatus, jqXHR) {
				createMagazineSuborderListHTMLData(data, '', divID);
				 ...
				 $('#orderSubList_'+parentjobno).show();
			});
...
function createMagazineSuborderListHTMLData(data, clearbody, divID) {
	...
	{	
		$.each(data,function(jobNo,jobDetails)
		{
			$.each(jobDetails,function(fieldKey, fieldValue)
			{
				...
				if(fieldKey!='Bkgnd_color' && fieldKey!='JobNo' && fieldKey!='Totalrows' && fieldKey!='Action_items' && fieldKey!='Deliverypritype' && fieldKey!='Revisioncount' && fieldKey!='Deliverytype' && fieldKey !='Order_type' && fieldKey!='Siteordercustomstatus')
				{
					var headTitle = $('<div />');
					if(fieldKey=='Job_no')
					{
	...
	$.each(data,function(jobNo,jobDetails)
	{
		...
		$.each(jobDetails,function(fieldKey, fieldValue)
		{
			...
			if(fieldKey!='Bkgnd_color' && fieldKey!='Status' && fieldKey!='JobNo' && fieldKey!='Totalrows' && fieldKey!='Action_items' && fieldKey!='Deliverypritype' && fieldKey!='Revisioncount'  && fieldKey!='Deliverytype' && fieldKey!='Order_type' && fieldKey!='Siteordercustomstatus')
			{
				if (fieldKey == 'Submit_by') {
					jobSubmittedBy = fieldValue;
				}
...
/**
 * Function (createCampaignListHTMLData) is used to  create CampaignList HTML
 * 
 * 
 * @return null.
*/

function createCampaignListHTMLData(data, clearbody) 
{
	data = $.parseJSON(data);
	...
	if(def===undefined)
	{
		...
		if(activeTabs == '4')
		{
			headingDiv.html("<h4 class='activeJobTab magazinePage'> <a class='' rel='activeAds' href='javascript:void(0);' id='activeAds'>Active Ads</a> <a rel='activeCampaign' href='javascript:void(0);' class='sel' id='activeCampaign'>Active Campaign</a> <a rel='activeMagazine' href='javascript:void(0);' class='' id='activeMagazine'>Active Magazine</a> </h4> <div class='rt'><div id='jobs_reload'><span id='JobCount'>Showing <span id='displaying'></span> of <span id='total'></span></span></div></div>");
		}
		else if(activeTabs == '3')
		{
			headingDiv.html("<h4 class='activeJobTab magazinePage'> <a class='' rel='activeAds' href='javascript:void(0);' id='activeAds'>Active Ads</a> <a rel='activeCampaign' href='javascript:void(0);' class='sel' id='activeCampaign'>Active Campaign</a> </h4> <div class='rt'><div id='jobs_reload'><span id='JobCount'>Showing <span id='displaying'></span> of <span id='total'></span></span></div></div>");
		}
		else if(activeTabs == '2')
		{
			headingDiv.html("<h4 class='activeJobTab magazinePage'> <a class='' rel='activeAds' href='javascript:void(0);' id='activeAds'>Active Ads</a> <a rel='activeMagazine' href='javascript:void(0);' class='' id='activeMagazine'>Active Magazine</a> </h4> <div class='rt'><div id='jobs_reload'><span id='JobCount'>Showing <span id='displaying'></span> of <span id='total'></span></span></div></div>");
		...
		//console.log(data)
		orderListDiv = $('<div/>');
		orderListDiv.attr('id','activeCampaign');
		orderListDiv.attr('class','new-order-list-cont');	
	...
	{	
		$.each(data,function(jobNo,jobDetails)
		{
			$.each(jobDetails,function(fieldKey, fieldValue)
			{
				...
				if(fieldKey!='Bkgnd_color' && fieldKey!='JobNo' && fieldKey!='Totalrows' && fieldKey!='Action_items' && fieldKey!='Deliverypritype' && fieldKey!='Revisioncount' && fieldKey!='Deliverytype' && fieldKey!='Suborderpageno' && fieldKey!='Campaignid')
				{
					headTitle = $('<div />');
					if(fieldKey=='Job_no')
					{
	...
	$.each(data,function(jobNo,jobDetails)
	{
		...
		$.each(jobDetails,function(fieldKey, fieldValue)
		{
			...
			if(fieldKey=='Campaignid')
            {
                    CampaignId = fieldValue;
            }
							
			if(fieldKey!='Bkgnd_color' && fieldKey!='Status' && fieldKey!='JobNo' && fieldKey!='Totalrows' && fieldKey!='Action_items' && fieldKey!='Deliverypritype' && fieldKey!='Revisioncount'  && fieldKey!='Deliverytype' && fieldKey!='Suborderpageno' && fieldKey!='Campaignid')
			{
				if (fieldKey == 'Submit_by') {
					jobSubmittedBy = fieldValue;
				}
		...
		jobDetails.Action_items = 'ADD_MORE_ORDERS,'+jobDetails.Action_items;
		var actionMenuDiv = $('<div/>');
		actionMenuDiv.attr('class','actions-col');
		actionMenuDivHtml = '<div class="open" title="Open Sub Order">Open</div>';
		actionMenuDivHtml = actionMenuDivHtml + '<div title="Details" class="details">Details</div>';
		actionMenuDivHtml = actionMenuDivHtml + '<div title="Actions" class="actions"  id="actionMenu_'+jobNo+'" rel="'+jobDetails.Action_items+'##'+jobSubmittedBy+'##'+CampaignId+'" >Actions</div>';
		...
		var actionMenuDivVer = $('<div/>');
		actionMenuDivVer.attr('class','action-btn');
		actionMenuDivHtmlVer ='<div id="actionMenuVer_'+jobNo+'" rel="'+jobDetails.Action_items+'##'+jobSubmittedBy+'##'+CampaignId+'" ><img src="../images/actions.png" width="19" height="67" border="0" align="absbottom" /></div>';
		actionMenuDivHtmlVer = actionMenuDivHtmlVer ;
		createActionMenuIdsVer =  createActionMenuIdsVer + 'actionMenuVer_'+jobNo+',';		
		actionMenuDivVer.html(actionMenuDivHtmlVer);
	...
	// creating Menu for each JOB
	showPopupForCampaignJobs(createActionMenuIdsVer, "popup");
	showPopupForCampaignJobs(createActionMenuIds, "popup");
	...
	$(".scroll-job-div .nano").bind("scroll-end", function(e){
		...
		if($("#isloading").val()=="false")
	    {
    		...
    		$.post('/view/campaignlist', 'page='+currentPage+'&loadmore=1', function(data,textStatus, jqXHR) {		
       			$('#loadmoreprogress').fadeOut();
            	loadMoreCampaignListHTMLData(data,1);
    		});
	...
	$('.content-left-box').each(function(){
		...
		$(this).find('.open').click(function(){
			...
			$.post('/view/campaignlist', {'parent_jobno':parentjobno}, function(data,textStatus, jqXHR) {
				createCampaignSuborderListHTMLData(data, '', divID);
...
function createCampaignSuborderListHTMLData(data, clearbody, divID) {
	...
	{	
		$.each(data,function(jobNo,jobDetails)
		{
			$.each(jobDetails,function(fieldKey, fieldValue)
			{
				...
				if(fieldKey!='Bkgnd_color' && fieldKey!='JobNo' && fieldKey!='Totalrows' && fieldKey!='Action_items' && fieldKey!='Deliverypritype' && fieldKey!='Revisioncount' && fieldKey!='Deliverytype' && fieldKey !='Order_type' && fieldKey!='Campaignid')
				{
					var headTitle = $('<div />');
					if(fieldKey=='Job_no')
					{
	...
	$.each(data,function(jobNo,jobDetails)
	{
		...
		$.each(jobDetails,function(fieldKey, fieldValue)
		{
			...
			if(fieldKey!='Bkgnd_color' && fieldKey!='Status' && fieldKey!='JobNo' && fieldKey!='Totalrows' && fieldKey!='Action_items' && fieldKey!='Deliverypritype' && fieldKey!='Revisioncount'  && fieldKey!='Deliverytype' && fieldKey!='Order_type' && fieldKey!='Campaignid')
			{
				if (fieldKey == 'Submit_by') {
					jobSubmittedBy = fieldValue;
				}
	...
	$('.new-order-sub-list-cont').find('.status-col').parent().children().each(function(){
		...
		if(!($(this).hasClass('actions-col')))
		{
			$(this).click(function(){
				...
				$('#sub_order_details_ver').html('');
				$.post('/view/campaignlist', {'parent_jobno':parentjobno}, function(data,textStatus, jqXHR) {
					createCampaignSuborderVerticalListHTMLData(data);
					 $('#sub_order_details_ver').show();
				});
			});
		}
...
function createCampaignSuborderVerticalListHTMLData(data)
{
	data = $.parseJSON(data);
