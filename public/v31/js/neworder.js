(function(jQuery){	
	...
	jQuery.fn.enableChildFields = function(){	
		...
		$("div[parent='"+name+"'][parent_value='depend'][order_type='"+order_type+"'] select").each(function(){
			if($("div[title='"+name+"']").css('display')!='none')
                        {
                                $(this).trigger('change');
                        }	
		});
	...
	jQuery.fn.onChangeMyCustomSelect = function(obj, parent_name, selected_val, order_type,selected_product_name){
		...
		if(obj.attr('name')=='Additional Versions' && selected_val>=1)
		{
			$("div[parent='"+parent_name+"'][order_type='"+order_type+"']").each(function(){
				$(this).fadeIn('fast');
				$(this).find('select').each(function(){
					if($(this).attr('name')=='Size Change Only')
					{
						$(this).val('Different Creative');
						$(this).parent().parent().parent().prev().html('Different Creative');
		...
		else if(obj.attr('name')=='Additional Versions')
		{
			$("div[parent='"+parent_name+"'][order_type='"+order_type+"']").each(function(){
				$(this).fadeOut('fast');
			});
...
//		alert(site);	 
		...
		if(obj.attr('name')=="Build Platform")
		{
		    if(obj.val()=='Templated Design')
		    {
			if($('[name="Company Logo"]:checked').val()=='Yes')
	        	{
                		$('[parent="Company Logo"]:first').show();

		        }
		        else
		        {
		                $('[parent="Company Logo"]:first').hide();
        		}
			$('div[title="Type"][order_type="Web"]').hide();

			$('[name="Click Tag"]').parent().parent().parent().parent().hide();
                        $('[name="Click Tag"]').parent().parent().parent().parent().prev().find('label').html('');

			$('<span id="filesmsg" style="position:absolute;margin-top:3px;padding-top:3px;font-weight:bold">* Add up to 3 images including customer logo</span>').appendTo($('.ax-button').parent());	

			$('div[tooltip="Additional Versions"][order_type="Web"]').parent().hide();
			$('div[tooltip="Additional Versions"][order_type="Web"]').find('select').val('0');
			$('div[tooltip="Additional Versions"][order_type="Web"]').find('select').trigger('change');
			$('div[title="Copy Translation Required"][order_type="Web"]').parent().hide();
			$('div[title="Type"][order_type="Web"]').removeAttr('mandatory');
			$('#56').find('option[value="Rich"]').attr('disabled','disabled');
			$('#56').find('option[value="Animated GIF"]').attr('disabled','disabled');
			$('#56').find('option[value="Animated GIF/Flash"]').attr('disabled','disabled');
			$('#56').find('option[value="Static"]').attr('disabled','disabled');

			$('#56').val('Animated Flash');
			$('#56').parent().parent().parent().prev().html("Animated Flash");
			$('#56').parent().parent().parent().prev().attr('class','custRef');
			$('#56').trigger('change');


			$( "#41 option" ).each(function() {
				if($( this ).text() == 'Follow Layout with Changes(AD2)')
				{
					$(this).attr('disabled','disabled');
				}
				else if($( this ).text() == 'Follow Layout(AD1)')
				{
					$(this).attr('disabled','disabled');
				}
				else if($( this ).text() == 'Full Artistic Discretion(AD3)')
				{
					$(this).attr('selected','selected');
					$('#41').parent().parent().parent().prev().html("Full Artistic Discretion(AD3)");
					$('#41').parent().parent().parent().prev().removeClass('select_grey_out');
				}
			});
			

			$("[name='Landing URL']").attr('placeholder','Landing URL');

			$('#14 option').filter(function(){
                                        return $(this).text() === '3 PM';
                                }).prop('disabled',false);

			$('#14 option').prop('disabled',true);

			$('#14 option').filter(function(){
                                        return $(this).text() === '6 Hour';
                                }).prop('disabled',false);

			$('#14 option').filter(function(){
                                        return $(this).text() === 'Overnight';
                                }).prop('disabled',false);
			...
			if(n>=5 && n<=10)
			{
				...
				$('[title="Delivery Date"] .custRef').html('6 Hour').removeClass('select_grey_out');
			...
			else
			{
    		    ...
    		    {  
			$('div[tooltip="Additional Versions"][order_type="Web"]').parent().show();
    		        $('div[title="Type"][order_type="Web"]').show();
			$('div[title="Copy Translation Required"][order_type="Web"]').parent().show();
    		        $('div[title="Type"][order_type="Web"]').attr('mandatory','1');
    		        $('#56').find('option[value="Rich"]').removeAttr('disabled');
			$('#56').find('option[value="Animated GIF"]').removeAttr('disabled');
                        $('#56').find('option[value="Animated GIF/Flash"]').removeAttr('disabled');
                        $('#56').find('option[value="Static"]').removeAttr('disabled');

			$('[name="Click Tag"]').parent().parent().parent().parent().show();
			$('[name="Click Tag"]').parent().parent().parent().parent().prev().find('label').html('Click Tag');
			$('#filesmsg').remove();

			$('[name="Company Name"]').parent().hide();
			...
			$( "#41 option" ).each(function() {
				...
				else if($( this ).text() == 'Full Artistic Discretion(AD3)')
				{
					...
					$('#41').parent().parent().parent().prev().html("Artistic Discretion");
					$('#41').parent().parent().parent().prev().addClass('select_grey_out');
				}
			});
		...
		if((parent_name=='Type' || parent_name.indexOf('Type_')!=-1) && selected_val.indexOf('(')==-1)
                {
                        $("div[parent='"+parent_name+"'][order_type='"+order_type+"'][parent_value='Custom']").each(function(){
                                $(this).fadeIn('fast');
                        });
                }
	}
		...
		if((parent_name=='Type' || parent_name.indexOf('Type_')!=-1) && selected_val.indexOf('(')==-1 && selected_val!='--Select--')
                {
                        $("div[parent='"+parent_name+"'][order_type='"+order_type+"'][parent_value='Custom']").each(function(){
                                $(this).fadeIn('fast');
                        });                
		...
		$("div[parent='"+parent_name+"'][parent_value='depend'][order_type='"+order_type+"']").each(function(){
			...
			//if($('#performAjaxCall').val()=='false') return;	

			if($('[name="Build Platform"]').val()=='Templated Design' && $(this).attr('title')=='Type')
        		{
		                return;
        		}	
			$(this).fadeIn('fast',function(){	
			if($('#performAjaxCall').val()=='false') {
				return;
			}
...
if(site=='Media Services(Auckland)' && ord_type=='Web' && f_name=='Type')
{
	...
	if($('[name="Build Platform"]').val()=='Templated Design')
	{
		$('div[title="Type"][order_type="Web"]').hide();
	}
		...
		/* Product based Deivery Configuration. It will tigger while selecting the output-type and type */
		...
		if(selected_val!='' && selected_val!='--Select--' && selected_val!=null && (parent_name == selected_product_name+version_val) )
		{
			$(this).calldeliveryoptions(obj,0);
		}
		var obj_parent_attr = obj.parent().parent().parent().parent().attr('parent');
		
		if( (parent_name=='Type' || (f_name=='Type' && parent_name.indexOf('_')!=-1) && (obj_parent_attr.indexOf(selected_product_name)!=-1 || obj_parent_attr.indexOf('Output Type')!=-1)) && selected_val!='' && selected_val!='--Select--' && selected_val!=null)
		{
			if(order_type=='Print')
			{
				$(this).calldeliveryoptions(obj,0);
		...
		else if((parent_name=='Output Type' || (f_name=='Output Type' && parent_name.indexOf('_')!=-1) && (obj_parent_attr.indexOf(selected_product_name)!=-1 || obj_parent_attr.indexOf('Output Type')!=-1)) && selected_val!='' && selected_val!='--Select--' && selected_val!=null)
		{
			if(order_type=='Web' || order_type=='Mobile'){
				$(this).calldeliveryoptions(obj,0);
			}
		}
	...
	/* Get product based delivery Date which is the highest all selected product */
	jQuery.fn.calldeliveryoptions = function(obj,flag){
		var request_filed = obj.attr('name');
		var site = $("#ord_form [name='Site']").val();
		var postValue = $(this).selectedSpec();
		var deliveryID = $("#14").val();
		
		$.post('/orders/configureddeliveries/',
				{'site':site, 'selectedSpecItems':postValue, 'request_filed':request_filed}, 
				function(data){
				...
				//$('#14').parent().parent().parent().parent().parent().find('[class="custRef"]').html('Delivery Date');
				if(deliveryID!="" && flag==1){
					$('#14').parent().parent().parent().parent().parent().find('[class="custRef"]').html(deliveryID);
					$('#14 option[value="'+deliveryID+'"]').attr("selected", "selected");
				}*/
		});
	};
	
	/* This function use selected value order type as print/web/mobile : Return json object */
	jQuery.fn.selectedSpec = function(){
		var selectedProductSpec = new Object();
		$("input[name='Order Type']:checked").each(function(){
			var ordtype = $(this).val();
			var prodname ='';
			if(ordtype=="Print"){
				var prodname = $('#order_spec_form_'+ordtype).find('[id="22"]').first().attr('name');
			}if(ordtype=="Web"){
				var prodname = $('#order_spec_form_'+ordtype).find('[id="49"]').first().attr('name');
			}if(ordtype=="Mobile"){
				var prodname = $('#order_spec_form_'+ordtype).find('[id="68"]').first().attr('name');
			}
			if(ordtype=="Print"){
				...
				version = 0;
				$('#order_spec_form_'+ordtype).find('[name="'+prodname+'"]').each(function(){
				...
				for(i=1;i<=version;i++)
				{
					var val = $('#order_spec_form_'+ordtype).find('[name="'+prodname+'_'+i+'"],[name="Asset Category_'+i+'"] option:selected').val();
				...
				for(i=1;i<=version;i++)
				{
					...
					var val = $('#order_spec_form_'+ordtype).find('[name="Output Type_'+i+'"] option:selected').val();
	...
	jQuery.fn.onChangeCustomSelect = function(obj, parent_name, selected_val, order_type){
		...
		if(parent_name=='Type' && selected_val.indexOf('(')==-1)
		{
			/*if(order_type=='Print')
			{
	...
	jQuery.fn.applyCustomSelect = function(){
		$("#ord_form .customSelect").each(function(){
			...
			$(this).find('select').change(function(){				
				...
				if(order_type =="Print"){
					product_name = $('#order_spec_form_'+order_type).find('[id="22"]').first().attr('name');
				}else if(order_type =="Web"){
					product_name = $('#order_spec_form_'+order_type).find('[id="49"]').first().attr('name');
				}else if(order_type =="Mobile"){
					product_name = $('#order_spec_form_'+order_type).find('[id="68"]').first().attr('name');
				}
				$(this).onChangeMyCustomSelect(obj, $(this).attr('name'), selected_val, order_type,product_name)
			})
	...
	jQuery.fn.cleanOrderFields = function(){		
		...
		//For Single OrderType End
			...
			if($(this).attr('checked')=='checked')
			{
				...
				$.doTimeout(2000,function(){
                                        $('#performAjaxCall').fadeOut('show').delay('20000').fadeIn('slow').val('true');
                                });
				if($('#performAjaxCall').val()=='false')
				{
				var sizechangeopt = $('#ord_form'+selected_val).find('[parent="Additional Versions"] [name="Size Change Only"]').val();
				if(sizechangeopt=='Different Size' || sizechangeopt=='Different Creative/Size' || (sizechangeopt=='--Select--' && $('#ord_form'+selected_val).find('[name="Additional Versions"]').val()>0))
				{
					$('#ord_form_multiver_'+selected_val).show('fast');
					$('#ord_form_multiver_'+selected_val).find('div[parent="Size Change Only"]').show('fast');
		...
		$("div[parent]").each(function(){
			...
			if($(this).attr('parent')!='' && (($(this).attr('parent_value')!=$("div[order_type='"+$(this).attr('order_type')+"'] [name='"+$(this).attr('parent')+"']").val() && $(this).attr('parent_value')!='depend')))
			{			
				if($("div[order_type='"+$(this).attr('order_type')+"'] [name='"+$(this).attr('parent')+"']").val()=='--Select--' || $("[name='Order Type'][value='"+$(this).attr('order_type')+"']").attr('checked')==undefined){
				...
				{
					if($("[name='Order Type'][value='"+$(this).attr('order_type')+"']").attr('checked')===undefined)
                        		{
						$("div[order_type='"+$(this).attr('order_type')+"'] [name='"+$(this).attr('parent')+"']").trigger('change');
					}
					else if($(this).attr('parent')!='' && ( $(this).attr('changeval')=='' || ($(this).attr('parent_value')!=$("div[order_type='"+$(this).attr('order_type')+"'] [name='"+$(this).attr('parent')+"']").val() && $(this).attr('parent_value')!='depend')))
		...
		$("[name='Size Change Only']").each(function(){
			...
			$(this).unbind().change(function(){			
				...
				{	
					...
					$("form[id='ord_form_multiver_"+ord_type+"']").find("div[ver][parent='Size Change Only'][order_type='"+ord_type+"']").each(function(){
						existing_versions++;
					});
					...
					if(selected_versions>existing_versions)
					{
						...
						for(mul=existing_versions; mul<selected_versions; mul++)
						{
							...
							cont_div_obj = '';
							cont_div_obj1 = '';
							$('#ord_form'+ord_type+' > .o-row').each(function(){
								$(this).find("div[order_type='"+ord_type+"'][multiversion='1']").each(function(){								
								...
								//{
									...
									{
										if($(this).is(':first-child') || clone_obj_name_org=='Output Type')
										{
							...
							$(this).find("div[order_type='"+ord_type+"'][multiversion='1']").each(function(){
								...
								//{							
									...
									if(clone_obj_name_org!='undefined'){
										...
										{
											clone_obj_inp.val(clone_obj_inp.parent().parent().parent().prev().html().replace('&amp;','&'));
										}
		...
		$("[name='Multiple Formats']").each(function(){		
			...
			$(this).unbind().change(function(){			
				...
				var ord_type = $(this).parent().attr('order_type');
				var parent = 'Additional Versions';		
		...
		$("#formSubmit").click(function(event){
			...
			var check_mandatory = $("[title='Tracking No']").attr('mandatory');
			if(!objRegExp.test($("[name='Tracking No']").val()) && (check_mandatory == '1'))
			{
			...
			errFlag = false;
			$("div[order_type]:visible").each(function(){
				$(this).find('select').each(function()
                                {
					if($(this).attr('name')=='Size Change Only' && $(this).parent().parent().parent().parent().is(':visible') && $(this).val()=='--Select--')
                                        {
                                                $(this).parent().parent().parent().parent().css('background-color','#FF0000');
                                                errFlag = true;
                                        }
                                        else if($(this).attr('name')=='Size Change Only')
                                        {
			...
			$("div[order_type][mandatory='1']:visible").each(function(){
				$(this).find('input').each(function()
				{
					...
					if($(this).attr('name')=='Width' || $(this).attr('name')=='Height' || $(this).attr('name')=='Depth')
					{
						if($(this).val()=='0')
						{
							$(this).css('background-color','#FF0000');
				...
				$(this).find('select').each(function()
				{
					...
					//alert($(this).attr('name')+'-'+$(this).parent().parent().parent().parent().is(':visible')+'-'+$(this).val());
			...
			$("#order_info_form").find("input:checked[name='Order Type']").each(function(){				
				...
				for(fe=0;fe<form_spec_details_arr.length;fe++)
				{
					...
					if(form_field_value[0].indexOf('%5B')!=-1)
					{
						form_field = $(this).val()+'['+form_field_value[0].substr(0,form_field_value[0].indexOf('%5B'))+'][]';
					}
					else
					{
		...
		$("#main_body select[name='Art Available on FTP']").on('change',function ()	{
			if($('#main_body select[id="Art Available on FTP"]').val()=='Yes')
			{
				$('#ordercomponents').hide();

				$("#order_info_form").find("#artfilesftpmoved").val('yes');
				$("#msgbox_text").html("<div>Waiting for File Upload to complete...<img src='/images/transparent_loading.gif' style='vertical-align:middle;' /></div>");
				$("#msgbox").fadeIn('fast',function(){
				$("#msgbox").css('display','table');
				});
			
				$.post("/orders/checkartavailableftpstatus",{'Site':$("input[name='Site']").val(),'trackno':$("input[name='Tracking No']").val()},function(data) {
					$("#msgbox").fadeOut('fast');
					if(data=='success'){
						$('#formSubmit').parent().show();
						$('.formsave').parent().hide();
					}
					else {
							alert("File Not Found on ftp");
							$('#formSubmit').parent().hide();
							$('.formsave').parent().show();
					}
				});
			}
			else
			{
				...
				$('#ordercomponents').show();

				$("#order_info_form").find("#artfilesftpmoved").val('No');
			}
		});
		
		$('.formsave').click(function(){$('#formSave').click();});
		
		$("#main_body select[name='Art Available on FTP']").change();
