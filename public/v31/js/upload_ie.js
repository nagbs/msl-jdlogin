...
{	
	...
	function formList(list, obj, settings)
    {
	    ...
	    tools.pBar.append($('<iframe frameborder="0" id="uploadForm_'+fileUploadFrameCnt+'" name="uploadForm_'+fileUploadFrameCnt+'" class="ax-progress-info" src="/upload_frame.php?up_id='+up_id+'" scrolling="no"></iframe>'));    	
	...
	$.fn.myFileUpload = function(options)
	{
		...
		return this.each(function(){
			...
			fileid = 'fileinput'+fileobj;

			fileobj++;

			
			var browse = $('<label for="'+fileid+'" id="filebutton"><a class="ax-button home" title="Add Files"><div class="button"><span>Add Files</span></div></a></label>');		
			
			$(this).find('[name="file[]"]').parent().prepend(browse);

			$(this).find('[name="file[]"]').attr('id',fileid);
			...
			$(this).find('[name="file[]"]').bind('change',function(e){		
				...
				//fileList = $(this).parent().find('ul.ax-file-list');
