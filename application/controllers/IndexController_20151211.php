	...
	public function forgotpwdAction()
	{
		if($this->_request->isPost())
		{
			...
			if ($rest_objForUser->response->status == '1')
			{
				...
				$sendermailid=$rest_objForUser->response->emailid;

				if(substr_count($sendermailid,",")>0)
				{
					$sendermailid = explode(",",$sendermailid);
				}
				...
				if(is_array($sendermailid))
				{
					foreach($sendermailid as $to_email)
					{
						$mail->addTo($to_email);
					}
				}
